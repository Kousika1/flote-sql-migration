...
const entities = [AdCampaigns, AdClicks, AdCredits, Adds, AdImpressions, AdminAlerts, AdPlacements, Ads, ArInternalMetadata, AuthenticatedAccounts, AverageTransactionFees, BitcoinAddressBackups, BitcoinAddresses, BitcoinPurchases, BitcoinWallets, Blocks, Boards, Channels, Comments, Contacts, Conversations, ConversationUsers, DeletedUserBackups, DeliveryAddresses, Events, FacebookImports, Files, Filters, HiddenHashtags, Invites, Invoices, IrrelevantWords, Likes, ListBlocks, ListMemberships, Lists, ListSubscriptions, Livestreams, LivestreamViewers, MediaThumbnails, Messages, MessageUploads, Metrics, Notifications, OldSenderKeys, PaymentAddresses, Payments, PostBoosts, Posts, PostUploads, Products, ProductInventories, PurchaseOrders, PurchaseOrderSupports, Purchases, Reports, Reposts, Rulings, SchemaMigrations, SenderKeys, ShippingRates, SubscriptionRequests, Subscriptions, SupportPayments, Supports, Tiers, TokenCredits, TransactionInputs, TransactionOutputs, Transactions, Unfollows, Uploads, UrlMetadata, UserLogs, Users, Views, Votes, WyreTransactions, ProductUploads];
...
function insertData(repository, name, table) {
  return new Promise(async (resolve, reject) => {
    ...
    const result = await pgConnection.query(`SELECT ${keys[name]} FROM ${table} WHERE id NOT IN (${mysqlData.map((i) => i.id).join(", ")})`);
...
async function migrateDataTables() {
  try {
    ...
    console.log("Error in Differential migration:", error);
  }
}
